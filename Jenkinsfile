    ...
    stages{
       stage('CheckOut'){
        ...
        agent { node { label 'master' } }    
            steps {    
              sh 'docker run --rm --name postgres-sme --network python-net -p 5432 -d -e POSTGRES_DB=teste -e POSTGRES_PASSWORD=adminadmin -e POSTGRES_USER=admin postgres'
            }    
        }
        stage('Prep'){
          agent { node { label 'master' } }    
